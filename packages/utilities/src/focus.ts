 ...
 */
export function focusFirstChild(rootElement: HTMLElement | null, bypassHiddenElements?: boolean): boolean {
...
export function getNextElement(
  ...
  const isCurrentElementVisible = checkElementVisibility(currentElement)
...
export function isElementTabbable(element: HTMLElement, checkTabIndex?: boolean): boolean {
  ...
  if (element && element.getAttribute) {
    ...
    if (tabIndexAttributeValue)
      tabIndex = Number.parseInt(tabIndexAttributeValue, 10)
  }
...
export function doesElementContainFocus(element: HTMLElement): boolean {
  ...
  const currentActiveElement: HTMLElement | undefined = document && (document.activeElement as HTMLElement)
