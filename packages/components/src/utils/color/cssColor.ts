...
function _hex6(str: string): IRGB | undefined {
  if (str[0] === '#' && str.length === 7 && /^#[\da-fA-F]{6}$/.test(str)) {
    return {
      r: Number.parseInt(str.slice(1, 3), 16),
      g: Number.parseInt(str.slice(3, 5), 16),
      b: Number.parseInt(str.slice(5, 7), 16),
      a: MAX_COLOR_ALPHA,
    }
  }
}
...
function _hex3(str: string): IRGB | undefined {
  if (str[0] === '#' && str.length === 4 && /^#[\da-fA-F]{3}$/.test(str)) {
    return {
      r: Number.parseInt(str[1] + str[1], 16),
      g: Number.parseInt(str[2] + str[2], 16),
      b: Number.parseInt(str[3] + str[3], 16),
      a: MAX_COLOR_ALPHA,
    }
  }
}
