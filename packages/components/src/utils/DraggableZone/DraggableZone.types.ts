...
export interface IDraggableZoneProps {
  /**
   ...
   */
  onStart?: (event: MouseEvent & TouchEvent, dragData: IDragData) => void

  /**
   * Callback for when the drag changes, while dragging
   */
  onDragChange?: (event: MouseEvent & TouchEvent, dragData: IDragData) => void

  /**
   * Callback for when dragging stops
   */
  onStop?: (event: MouseEvent & TouchEvent, dragData: IDragData) => void
}
