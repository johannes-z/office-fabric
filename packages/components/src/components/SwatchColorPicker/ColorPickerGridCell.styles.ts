...
import { HighContrastSelector, getFocusStyle } from '@fluentui-vue/style-utilities'
...
export function getStyles(props: IColorPickerGridCellStyleProps): IColorPickerGridCellStyles {
  const {
    theme,
    disabled,
    selected,
  ...
  const calculatedBorderWidth = borderWidth || (width < CELL_BORDER_BREAKPOINT
    ? SMALL_BORDER
    : LARGE_BORDER)
