...
import { PanelType, type IPanelStyleProps, type IPanelStyles, type IPanelProps } from ".";
import { defineComponent, toRefs, type PropType, ref, computed, h, type VNodeRef, onMounted, onBeforeUnmount, watch, type VNode, type VNodeArrayChildren } from "vue";
import { makeStylingProps } from '@/utils'
import { Layer, type IFocusTrapZoneProps, type ILayerProps, FocusTrapZone, IconButton } from "..";
...
export const PanelBase = defineComponent({
  ...
  setup(props, { attrs, slots, expose }) {
    ...
    const isLeft = computed(() => (type.value === PanelType.smallFixedNear || type.value === PanelType.customNear ? true : false));
    ...
    const updateFooterPosition = () => {
        if (scrollableContent.value) {
            ...
            isFooterSticky.value = height < innerHeight ? true : false;
        }
    };
    ...
    return () => 
    {
        ...
        return h(Layer, {
            ...
            h(Popup, {
                ...
                enableAriaHiddenSiblings: isPanelOpen.value ? true : false,
                ...popupProps.value
            }, [
                h('div', {
